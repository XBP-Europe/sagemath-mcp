...
| container image | GHCR, `ghcr.io/xbp-europe/sagemath-mcp` | a **Cosign signature**, plus **SLSA provenance** and an **SPDX SBOM** pushed to the registry as attestations on the image digest |
| `sagemath-mcp-source.spdx.json`, `sagemath-mcp-image.spdx.json` | assets on the GitHub release | the same SBOMs as files, for consumers who do not read registry attestations |
...
on *that tag* — not merely by someone with push access to GHCR.
