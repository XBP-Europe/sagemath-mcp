...
The claim above -- that moving to the `uv` ecosystem fixes this -- is only
half true, and the first Dependabot PR after the switch showed it. #152 came
from the `uv` ecosystem, updated **both** `uv.lock` and the export, and still
failed `test_the_export_matches_the_lockfile`.

Measured: its committed export has 2,975 requirement lines, while
`uv export --frozen --extra passagemath --no-dev --no-emit-project` run
against *its own lock* produces 2,802. Dependabot regenerates the file
without the project's flags, and it has no way to learn them.

So what the switch fixed is coherence: the lock is now updated too, and an
update means something. What it did not fix is the red -- every Dependabot
Python PR will still fail the drift test until the export is regenerated by
hand with `make passagemath-lock`.

That leaves the original complaint standing in weaker form, and the reason it
mattered has not changed: a dependency PR that is reliably red is one a
reviewer stops reading.

The options, none of them free:

- **Accept the manual step.** Documented in CONTRIBUTING, named by the test's
  failure message. Costs one command per bump and keeps the weekly red.
- **Regenerate on the branch from CI.** Needs a workflow with
  `contents: write` reacting to Dependabot, whose PRs run with a read-only
  token by design; the routes around that are `pull_request_target` or a
  stored PAT.
- **Do the updates ourselves on a schedule** -- `uv lock --upgrade`,
  `make passagemath-lock`, open a PR -- and turn the Dependabot uv ecosystem
  off. Fully coherent, and #153 proved the sequence works by hand. The wrinkle
  is that a PR opened with `GITHUB_TOKEN` does not trigger other workflows, so
  CI would need a deliberate trigger.
