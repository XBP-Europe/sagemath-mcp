...
probe and a documentation lookup. As of 2026-09-21 the suite collects 1,375
tests; 1,209 of them run without Sage, at 100% statement and branch coverage,
and the rest need the container, where the same 1,375 are collected and the
...
[REVIEW_ACTIONS.md](REVIEW_ACTIONS.md) — 93 items, each with its reproduction,
...
resolved on 2026-08-24 (below).

That count is derived by `tests/test_docs_counts.py` rather than maintained by
hand; it said 34 for long enough to be wrong by a factor of nearly three.
...
`tests/test_sage_doctest_corpus.py` — accepted 98.59% of what is in scope as
of 2026-08-15, when
...
failed that test has been fixed. **Re-measured 2026-09-21 on `main`:** 98.8908%
(370,323 accepted, 4,153 refused, 58,268 out of scope by design) on SageMath
...
the prose here and that file disagree -- these numbers went two releases stale
before anything noticed. The ledger of what each step cost or won back is in
...
- [x] **Passagemath runtime for install footprint.** *Done, 2026-09-07; pin
      ...
      runs the whole suite plus the doctest corpus sweep against it (98.92%
      acceptance on 10.8.11 versus 98.83% on monolithic 10.9, as measured on
      2026-09-19; the current pair is in `doctest-corpus-stats.md`). The pin is exact
