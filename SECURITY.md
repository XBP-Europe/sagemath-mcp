...
below name v0.9.1, the current release. They were last run end to end against
v0.9.0 (2026-09-24), and are re-run against each release after it publishes.
...
cosign verify ghcr.io/xbp-europe/sagemath-mcp:0.9.1 \
...
Releases publish four tags: `v0.9.1` (the git ref), `0.9.1` (pin a patch),
...
```bash
gh attestation verify oci://ghcr.io/xbp-europe/sagemath-mcp:0.9.1 --owner XBP-Europe
gh attestation verify sagemath_mcp-0.9.1-py3-none-any.whl --owner XBP-Europe
```
