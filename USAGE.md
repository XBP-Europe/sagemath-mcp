...
- **Caller code is checked against an allowlist**, so a name works only if SageMath preloads it for mathematics, it is a safe builtin, or your own code defined it — including earlier in the same session. Anything else is refused, and the message names the fix where there is one: a name models tend to invent (`isprime`, `partitions`, `symbols`, `bessel_J_zeros`) is answered with the SageMath spelling, and a runtime error with a Sage-specific cause carries a hint — a Python `float` from `numerical_integral` has no `.n()` (wrap it in `N(...)`), `QQ('1e-3')` does not parse scientific notation (use `RR('1e-3')`), and a 53-bit value cannot yield 20 digits (compute at higher precision from the start). The AST validator runs on top of that (see [Security Sandbox](#security-sandbox)).
...
| Names that write, fetch or display | `oeis` (queries oeis.org), `install_doc`, `show`, `view`, `animate`, `html`, `search_src`, `search_doc`, `reference`, `Profiler` --- each demonstrated a file written, a network request or the installation read. `latex` is the exception and is **callable but not reachable into**: `latex(expr)` builds a string and is offered, while `latex.eval()` and `latex.has_file()` run a LaTeX toolchain, so every attribute of it is refused rather than a list of the dangerous ones (item 92). Plot tools are unaffected: they render through `.savefig(BytesIO)`, and LaTeX output imports `latex` from `sage.all` directly rather than from the caller namespace |
...
| The `sage` package itself | Caller code may not traverse it **at all** --- not `sage.misc.lazy_import.LazyImport(...)`, not `sage.functions.log.exp(1)`, not a bare `sage`. Until September 2026 the guard was a list of dangerous path segments, and ten of the thirty modules the worker classifies as dangerous had none listed: `sage.misc.lazy_import.LazyImport('os','system')('id')` executed while the bare `LazyImport` was correctly refused. Any list of segments is one Sage release behind, so the root is refused instead. Everything you need has a direct spelling --- write `exp(1)`, not `sage.functions.log.exp(1)` --- and the refusal message says so. It costs about 950 of SageMath's own doctest examples, priced deliberately |
| Forbidden modules | **Every** attribute of `os`, `sys`, `subprocess`, `shutil`, `socket`, `pathlib`, `builtins`, `operator`, `warnings`, `pari`, `oeis` --- at any depth under any offered root, so `codes.databases.os` is caught as well as a bare `os` (a `sage`-rooted spelling is refused one step earlier, by the row above). `pari` is the PARI *library* interface, which the external-CAS scrub missed because it comes from `sage.libs.pari`; `pari('system("id")')` ran a shell command |
| Sage sub-packages that execute | `cython`, `persist`, `remote_file`, `interfaces`, `inline_fortran`, `repl`, `package`, `temporary_file`, `attached_files`, `explain_pickle`, `edit_module`, `dev_tools`, `trace`, `sh` --- at any depth. Blocked as a *path*, so `A.trace()` is still ordinary mathematics and is not refused. Since the root rule above, a `sage`-rooted spelling like `sage.misc.trace.trace(...)` is caught one step earlier; this row still governs the same names under any other offered root |
...
- **`SecurityViolation` on ordinary-looking code**: caller code is checked
  ...
  | `show`, `view`, `html`, `animate`, `oeis` | Write to disk, launch a viewer or reach the network. **Use the plot tools instead** — `plot_expression` and friends return rendered MCP image content (PNG or SVG) the client displays, which is what you want over an MCP connection anyway. |

  The specialised tools cover most of what people reach for these for.
- **`Reaching into the 'sage' module is not permitted`**: write the name
  directly. `exp(1)` rather than `sage.functions.log.exp(1)`, `Integer(5)`
  rather than `sage.rings.integer.Integer(5)`. Everything SageMath preloads is
  already offered to you by its plain name, so the dotted spelling buys nothing
  and the module tree is the route a sandbox escape took in September 2026. The
  same applies to tool parameters, including fragments written in Sage-only
  syntax such as `[a..b]`.
