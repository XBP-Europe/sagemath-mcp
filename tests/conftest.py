...
class FakeContext:
    """Stub MCP context that records progress events.

    Deliberately has no `info`/`warning`/`error`/`debug`: the server sends no
    MCP log notifications. The capability is deprecated as of the 2026-07-28
    protocol (SEP-2577), is dropped on that era unless the client opts in per
    request, and every message it carried repeated a result or error the
    caller already receives. A tool that starts logging again fails here with
    an AttributeError rather than passing quietly.
    """
